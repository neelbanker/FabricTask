...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/manufacturer.product.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/agency.product.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/retailer.product.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/distributor.product.com/ca && ls *_sk)
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.product.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
      ...
      export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/manufacturer.product.com/ca && ls *_sk)
      export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/agency.product.com/ca && ls *_sk)
      export BYFN_CA3_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/retailer.product.com/ca && ls *_sk)
      export BYFN_CA4_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/distributor.product.com/ca && ls *_sk)
    ...
    docker-compose $COMPOSE_FILES stop orderer.product.com
    docker cp -a orderer.product.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.product.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.product.com

    for PEER in peer0.manufacturer.product.com peer1.manufacturer.product.com peer0.agency.product.com peer1.agency.product.com; do
...
function networkDown() {
  # stop org3 containers also in addition to manufacturer and agency, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the product
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/manufacturer.product.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/agency.product.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/retailer.product.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/distributor.product.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
...
# and two Peer Orgs (``Manufacturer`` & ``Agency``) each managing and maintaining two peer nodes.
# This file also specifies a consortium - ``SampleConsortium`` - consisting of our
# two Peer Orgs.  Pay specific attention to the "Profiles" section at the top of
# this file.  You will notice that we have two unique headers. One for the orderer genesis
# block - ``FourOrgsOrdererGenesis`` - and one for our channel - ``FourOrgsChannel``.
# These headers are important, as we will pass them in as arguments when we create
# our artifacts.  This file also contains two additional specifications that are worth
# noting.  Firstly, we specify the anchor peers for each Peer Org
# (``peer0.manufacturer.product.com`` & ``peer0.agency.product.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for AgencyMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/AgencyMSPanchors.tx -channelID $CHANNEL_NAME -asOrg AgencyMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for AgencyMSP..."
    exit 1
  fi
